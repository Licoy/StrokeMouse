...
# hdiutil on shared CI runners can hit transient "Resource temporarily unavailable".
hdiutil_retry() {
  local attempts="${HDIUTIL_RETRIES:-3}"
  local delay=2
  local n=1
  local status=0
  while (( n <= attempts )); do
    ...
    status=$?
    if (( n == attempts )); then
      echo "hdiutil $* failed after ${attempts} attempts (exit $status)" >&2
      return "$status"
    fi
    echo "hdiutil $* failed (exit $status); retry ${n}/${attempts} in ${delay}s..." >&2
    sleep "$delay"
    delay=$((delay * 2))
    n=$((n + 1))
  done
...
DMG_MOUNT="$(mktemp -d)"
hdiutil_retry attach "$DMG" -readonly -nobrowse -mountpoint "$DMG_MOUNT"
