...
macOS 鼠标与触控板手势自定义工具。可按住鼠标键或单个修饰键绘制轨迹，也可使用实验性的多指直接触控；匹配后执行快捷键、打开应用、窗口操作、媒体键、Shell / AppleScript 等。支持**全局或指定 App** 生效，手势配置可**导入导出**，本地运行、菜单栏常驻。
...
- **修饰键绘制**：按住一个 Fn / Control / Option / Shift / Command（默认 Fn）后移动指针绘制；必须精确按住单个支持键，额外修饰键会取消本次识别
- **实验性直接触控**：内置触控板支持 34 类三至五指轻触 / 双击 / 四向滑动，以及二至五指捏合 / 张开 / 顺逆时针旋转
- **触控板总开关**：可随时关闭直接触控通道而不删除已配置手势；私有后端故障只降级触控通道，不影响鼠标与修饰键绘制
...
直接触控通过运行时 `dlopen` / `dlsym` 加载 Apple 未公开的 `MultitouchSupport`，不静态链接该私有框架。它可能在 macOS 更新后失效；缺少框架、符号、设备或设备启动失败都会显示为触控通道故障，鼠标与修饰键绘制仍可继续使用，不会模拟回退或静默重试。

StrokeMouse 不会拦截触控板原生事件，因此系统手势可能与绑定动作同时发生。应用也不会保存或记录原始触点轨迹。第一次保存、启用或导入已启用的直接触控配置时会显示实验风险确认；之后可用总开关暂停直接触控而不删除配置。
...
4. 选择一种输入方式：按住鼠标触发键绘制、按住单个修饰键绘制，或完成已配置的直接触控手势
5. 匹配成功后执行绑定动作  
...
> **修饰键绘制**：修饰键监听为 listen-only，不吞键盘事件；短路径不会执行动作。轨迹来自系统指针位置，因此可以用触控板或鼠标移动指针。StrokeMouse 不会阻止修饰键本身对当前 App 的影响。
...
- `dlopen` / `dlsym` 动态加载 `MultitouchSupport`（实验性直接触控，不静态链接）
- JSON 配置持久化
