...
struct GestureEditorView: View {
    ...
    @State private var scopeBundleIds = ""
    ...
    init(profile: GestureProfile, onSave: @escaping (GestureProfile) -> Void, onCancel: @escaping () -> Void) {
        ...
        case .apps(let ids):
            ...
            _scopeBundleIds = State(initialValue: ids.joined(separator: ", "))
        }
        self.onSave = onSave
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section(L10n.string("editor.scope")) {
                    Toggle(L10n.string("editor.scopeGlobal"), isOn: $scopeIsGlobal)
                    if !scopeIsGlobal {
                        TextField(L10n.string("editor.bundleIds"), text: $scopeBundleIds, axis: .vertical)
                            .lineLimit(2...4)
                        Text(L10n.string("editor.bundleIdsHelp"))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    private func commitAndSave() {
        ...
        if scopeIsGlobal {
            ...
            let ids = scopeBundleIds
                .split(whereSeparator: { $0 == "," || $0 == "\n" || $0 == " " })
                .map(String.init)
                .filter { !$0.isEmpty }
            profile.scope = .apps(ids)
        }
        onSave(profile)
    }
}
